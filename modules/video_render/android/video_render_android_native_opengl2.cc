...
/*  bool isAttached = false;
  ...
  if(my_workerthread == rtc::Thread::Current()) {
	callBack_Test_Method("OpenGl2::UseOpenGL2 = worker == true");
  	env = GetThreadEnv(true);
}
  else {
	callBack_Test_Method("OpenGl2::UseOpenGL2 = client == true");
	env = GetThreadEnv(false);
}

  if (g_jvm->GetEnv((void**) &env, JNI_VERSION_1_4) != JNI_OK) {
  callBack_Test_Method("UseOpenGL2 != JNI_OK");
    // try to attach the thread and get the env
    // Attach this thread to JVM
    jint res = g_jvm->AttachCurrentThread(&env, NULL);
  callBack_Test_Method("UseOpenGL2 = g_jvm->AttachCurrentThread");
    ...
    if ((res < 0) || !env) {
      WEBRTC_TRACE(
          ...
          res, env);
  callBack_Test_Method("RendererAndroid(): Could not attach thread to JVM");
      return false;
    }
  callBack_Test_Method("UseOpenGL2 = isAttached = true");
    isAttached = true;
  }*/


  // get the renderer class
  jclass javaRenderClassLocal = ats.env()->FindClass("org/webrtc/videoengine/ViEAndroidGLES20");
  ...
  jmethodID cidUseOpenGL = ats.env()->GetStaticMethodID(javaRenderClassLocal,
                                                  "UseOpenGL2",
                                                  "(Ljava/lang/Object;)Z");
  ...
  jboolean res = ats.env()->CallStaticBooleanMethod(javaRenderClassLocal, cidUseOpenGL, (jobject) window); 
...
/*  bool isAttached = false;
  ...
  jclass javaRenderClassLocal =
      ats.env()->FindClass("org/webrtc/videoengine/ViEAndroidGLES20");
  if (!javaRenderClassLocal) {
    WEBRTC_TRACE(kTraceError, kTraceVideoRenderer, _id,
  ...
  _javaRenderObj = ats.env()->NewGlobalRef(_ptrWindow);
  if (!_javaRenderObj) {
    WEBRTC_TRACE(
        kTraceError,
        kTraceVideoRenderer,
...
/*    bool isAttached = false;
    ...
    if (ats.env() && _deRegisterNativeCID) {
      ats.env()->CallVoidMethod(_javaRenderObj, _deRegisterNativeCID);
    }
...
/*  bool isAttached = false;
  ...
  _redrawCid = ats.env()->GetMethodID(javaRenderClass, "ReDraw", "()V");
  if (_redrawCid == NULL) {
    WEBRTC_TRACE(kTraceError, kTraceVideoRenderer, _id,
  ...
  _registerNativeCID = ats.env()->GetMethodID(javaRenderClass,
  ...
  _deRegisterNativeCID = ats.env()->GetMethodID(javaRenderClass,
  ...
  if (ats.env()->RegisterNatives(javaRenderClass, nativeFunctions, 2) == 0) {
  ...
  ats.env()->CallVoidMethod(_javaRenderObj, _registerNativeCID, (jlong) this);
