...
VideoRenderAndroid::AddIncomingRenderStream(const uint32_t streamId,
  ...
  callBack_Test_Method("renderModule_->AddIncomingRenderStream");

  AndroidStream* renderStream = NULL;
  AndroidStreamMap::iterator item = _streamsMap.find(streamId);
callBack_Test_Method("item = _streamsMap.find");
  ...
  renderStream = CreateAndroidRenderChannel(streamId, zOrder, left, top,
                                            right, bottom, *this);
  if (renderStream) {
callBack_Test_Method("streamsMap[streamId] = renderStream");
    _streamsMap[streamId] = renderStream;
  }
  else {
    WEBRTC_TRACE(kTraceError, kTraceVideoRenderer, _id,
                 "(%s:%d): renderStream is NULL", __FUNCTION__, __LINE__);
callBack_Test_Method("renderStream is NULL");
    return NULL;
  }
callBack_Test_Method("return renderStream");
  return renderStream;
}
