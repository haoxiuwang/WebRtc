...
/*    if (g_jvm->GetEnv((void**)&env, JNI_VERSION_1_4) != JNI_OK) {
    ...
    jmethodID rid = ats.env()->GetMethodID(_javaRenderClass,
                                   "RemoveCallback", "()V");
    if (rid != NULL)
	ats.env()->CallVoidMethod(_javaRenderObj, rid);
...
//    JNIEnv* env = NULL;
//    g_jvm->GetEnv((void**)&env, JNI_VERSION_1_4);
/*  if(my_workerthread == rtc::Thread::Current()) {
	callBack_Test_Method("surface::init = worker == true");
  	env = GetThreadEnv(true);
}
  else {
	callBack_Test_Method("surface::init = client == true");
	env = GetThreadEnv(false);
}*/
...
/*  if (g_jvm->GetEnv((void**)&env, JNI_VERSION_1_4) != JNI_OK) {
    ...
    if ((res < 0) || !env) {
      WEBRTC_TRACE(kTraceError,
                   ...
                   res,
                   env);
  callBack_Test_Method("Could not attach thread to JVM");
      return -1;
    }
    isAttached = true;
  }*/
  ...
  jclass javaRenderClassLocal = ats.env()->FindClass("org/webrtc/videoengine/ViESurfaceRenderer");
  ...
  _javaRenderClass =
      reinterpret_cast<jclass>(ats.env()->NewGlobalRef(javaRenderClassLocal));
  if (!_javaRenderClass) {
    WEBRTC_TRACE(kTraceError,
                 kTraceVideoRenderer,
                 _id,
  ...
  ats.env()->DeleteLocalRef(javaRenderClassLocal);

  // get the method ID for the constructor
  jmethodID cid = ats.env()->GetMethodID(_javaRenderClass,
                                   "<init>",
  ...
  jobject javaRenderObjLocal = ats.env()->NewObject(_javaRenderClass,
                                              cid,
					      _ptrWindow);
  ...
  _javaRenderObj = ats.env()->NewGlobalRef(javaRenderObjLocal);
  if (!_javaRenderObj) {
    WEBRTC_TRACE(kTraceError,
                 kTraceVideoRenderer,
                 _id,
...
AndroidSurfaceViewRenderer::CreateAndroidRenderChannel(
  ...
  WEBRTC_TRACE(kTraceDebug,
               ...
               __FUNCTION__,
               streamId);
  callBack_Test_Method("AndroidSurfaceViewRenderer::CreateAndroidRenderChannel");
...
int32_t AndroidSurfaceViewChannel::Init(
    ...
    const float right,
    const float bottom) {
callBack_Test_Method("AndroidSurfaceViewChannel::Init");
  WEBRTC_TRACE(kTraceDebug,
               kTraceVideoRenderer,
               _id,
  ...
  if (!_jvm) {
    WEBRTC_TRACE(kTraceError,
                 ...
                 "%s: Not a valid Java VM pointer",
                 __FUNCTION__);
callBack_Test_Method("Not a valid Java VM pointer");
...
//_jvm->GetEnv((void**)&env, JNI_VERSION_1_4);
/*  if(my_workerthread == rtc::Thread::Current()) {
	callBack_Test_Method("SurfaceChannel::Init = worker == true");
  	env = GetThreadEnv(true);
}
  else {
	callBack_Test_Method("SurfaceChannel::Init = client == true");
	env = GetThreadEnv(false);
}*/
...
callBack_Test_Method("ats.env()->FindClass(org/webrtc/videoengine/ViESurfaceRenderer");
  jclass javaRenderClass =
      ats.env()->FindClass("org/webrtc/videoengine/ViESurfaceRenderer");
  if (!javaRenderClass) {
    WEBRTC_TRACE(kTraceError,
                 kTraceVideoRenderer,
                 _id,
...
if(ats.Attached())callBack_Test_Method("ats attached!");
callBack_Test_Method("could not find ViESurfaceRenderer");
    return -1;
  }

  // get the method ID for the CreateIntArray
  _createByteBufferCid =
      ats.env()->GetMethodID(javaRenderClass,
                       "CreateByteBuffer",
  ...
  if (_createByteBufferCid == NULL) {
    WEBRTC_TRACE(kTraceError,
                 ...
                 __FUNCTION__);
callBack_Test_Method("could not get CreateByteBuffer ID");
    return -1; /* exception thrown */
  }

  // get the method ID for the DrawByteBuffer function
  _drawByteBufferCid = ats.env()->GetMethodID(javaRenderClass,
                                        "DrawByteBuffer",
                                        "()V");
  ...
  _setCoordinatesCid = ats.env()->GetMethodID(javaRenderClass,
                                        "SetCoordinates",
                                        "(FFFF)V");
  ...
  ats.env()->CallVoidMethod(_javaRenderObj, _setCoordinatesCid,
...
/*  if (isAttached) {
  ...
  WEBRTC_TRACE(kTraceDebug,
               ...
               __FUNCTION__);
callBack_Test_Method("AndroidSurfaceViewChannel done");
  return 0;
}
